...
public class DbHelper extends SQLiteOpenHelper 
{
    ...
    {
        ErrorHelper.Log("Before DB creation.");
    	...
    	db.execSQL("create table if not exists " + FILE_TABLE + " (" +
                ...
                PLANE_COL + " integer default " + R.drawable.paper_plane_24 + ", " +
                NAME_COL + " text, " +
        ...
        ErrorHelper.Log("After DB creation.");
    }

    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion)
    {
      ErrorHelper.Log("Upgrading database from version " + oldVersion + " to " + newVersion + ", which will destroy all old data");
    ...
    public static void updateFileList(ArrayListEx<RuleModel> arr)
    {
        try
        {
            ...
            for (RuleModel m : arr)
            {
                ...
                insert.bindLong(2, m.getRejectCallsFromIt() ? 1 : 0);
    ...
    public static void insertRejectedCall(String number)
    {
        try
        {
            String sql = "INSERT OR IGNORE INTO " + CALLS_TABLE + "(caller, date) VALUES (?, ?)";

            myDb.beginTransaction();
            SQLiteStatement insert = myDb.compileStatement(sql);

            insert.bindString(1, number);
            insert.bindString(2, getTimeStamp());
        ...
        {
            ErrorHelper.ShowError("insertRejectedCall(number) exception (" + getTimeStamp() + "): ", ex);
        }
        finally
        {
            myDb.endTransaction();
    ...
    public static int getRejectedCallsCount()
    {
        try
        {
            final String sql = "select count (*) from " +  CALLS_TABLE;
            return (int) DatabaseUtils.longForQuery(myDb, sql, null);
        }
        catch(Exception ex)
        {
            ErrorHelper.ShowError("getRejectedCallsCount() exception (" + getTimeStamp() + "): ", ex);
        }
        return -1;
    }

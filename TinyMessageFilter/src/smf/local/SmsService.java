    ...
    private final class ServiceHandler extends Handler
    {
                ...
                if (intent != null)
                {
                    ...
                    try
                    {
                        ...
                        if ( extras != null )
                        {
                            ...
                            StringBuilder usedRule = new StringBuilder();

                            // If BlackList is not used we should not pass here!
                            if (sms.isInBlackList() || sms.hasBadContent(usedRule))
                            {
                                if (usedRule.length() > 0)
                                    ErrorHelper.Log("applied rule: [" + usedRule + "]");
                                else
        ...
        private void keepAndNotify(SmsMessageEx sms, boolean isSpam)
        {
            ...
            if (!sms.isKnown() && SmsReceiver.userPrefs.notifyOfBlockedMessages())
            {
                ...
                sms.notifyOfBlockedSms(R.drawable.plane_black);
            }

            if (!isSpam)// && SmsReceiver.userPrefs.showCustomIcons())
            {
                ErrorHelper.Log("abortBroadcast for custom notify");
                sms.notify(sms.file != null ?  sms.file.getPlaneId() : R.drawable.paper_plane_24);
            }
        }
    }
}
